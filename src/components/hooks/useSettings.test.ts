...
describe('useSettings', () => {
  ...
  it('setFlashActive(false) toggles flashActivePlayes off', () => {
    const { result } = renderHook(() => useSettings());
    ...
    expect(result.current.settings.flashActivePlayes).toBe(false);
  });
  ...
  it('setFlashActive(true) toggles it back on without mutating previous state', () => {
    ...
    expect(result.current.settings.flashActivePlayes).toBe(true);
    // previous state reference should not have been mutated
    expect(firstSettings.flashActivePlayes).toBe(false);
  });
});
